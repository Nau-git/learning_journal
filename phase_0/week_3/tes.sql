...
SELECT name,company, power, 
RANK() OVER(ORDER BY power DESC) AS [Rank], -- ada lompat
DENSE_RANK() OVER(ORDER BY power DESC) AS [Dense Rank], -- ga lompat, bisa doble
ROW_NUMBER() OVER(ORDER BY power DESC) AS [Row Number]  -- ga lompat, ga doble
FROM Cars;
