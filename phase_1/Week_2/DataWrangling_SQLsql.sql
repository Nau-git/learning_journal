...
-- founded_at pake format tanggal US MM/DD/10
-- founded_at_cleaned pake format YYYY/MM/DD


SELECT founded_at,
	convert(founded_at, DATE),
	founded_at_clean,
    convert(founded_at_clean, date)
from crunchbase_companies_clean_data;
-- founded_at setelah diconvert malah jadi messed up
-- makanya harus diubah dulu pake str_to_date
...
-- di bawah bakal return error
alter table crunchbase_companies_clean_data
modify COLUMN founded_at, date;

-- solusinya:
-- step 1: siapin kolom baru null
-- step 2: copas + str_to_date
-- step 3: drop kolom lama
-- NOTES: solusi ini bakal ngerubah isi table secara permanen. Pastikan udah dapet izin sebelum melakukan ini
# step 1 :
alter table crunchbase_companies_clean_data
...
--
select start_time, 
	left(start_time, 10) as sliced_date,
...
from dc_bikeshare_q1_2012;
-- partition by itu kaya groupby python
...
SELECT start_terminal,
       start_time,
       duration_seconds,
       NTILE(4) OVER (partition BY start_terminal order by start_time) AS quartile,
       NTILE(5) OVER (partition BY start_terminal order by start_time) AS quantile
FROM dc_bikeshare_q1_2012;
...
-- LAG ini mirip diff() di python pandas
...
SELECT start_terminal,
       ...
       duration_seconds - LAG(duration_seconds, 1) OVER
         (PARTITION BY start_terminal ORDER BY duration_seconds) AS diff
FROM dc_bikeshare_q1_2012
ORDER BY start_terminal, duration_seconds
